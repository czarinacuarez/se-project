                        ...
                        <div class="w-full md:w-1/2">
                            <form class="flex items-center">
                                ...
                                <div class="relative w-full">
                                    ...
                                    <input type="text" id="simple-search" placeholder="Search for school" required="" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full pl-10 p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                </div>
                            </form>
                        </div>
                        <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                            <button type="button" id="createProductButton" data-modal-target="schoolModal" data-modal-toggle="schoolModal" class="flex items-center justify-center py-2 px-4 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    <th scope="col" class="p-4">School Initial</th>
                                    <th scope="col" class="p-4">School</th>
                                    <th scope="col" class="p-4">Points</th>
                                    <th scope="col" class="p-4">Actions</th>
                                </tr>
                            </thead>
                            @include('crud_components.school_program_table')
                        </table>
                    </div>
                </div>
            </div>
        ...
        <form method="POST" action="{{ url('update_school', ['id' => $lists->id ]) }}" enctype="multipart/form-data" id="drawer-update-product-{{ $lists->id }}" class="fixed top-0 left-0 z-40 w-full h-screen max-w-xl p-4 overflow-y-auto transition-transform -translate-x-full bg-white dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-update-product-label" aria-hidden="true">
            ...
            <div>
                <div class="grid grid-cols-2 gap-4 sm:col-span-2">
                    <div>
                        <label for="school_initials" class="block text-sm font-medium text-gray-900 dark:text-white">School Initial</label>
                        <input type="text" name="school_initials" id="school_initials" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" value="{{ $lists->school_initials }}" placeholder="Type product name" required="">
                    </div>  
                    <div>
                        <label for="school_name" class="block text-sm font-medium text-gray-900 dark:text-white">School Name</label>
                        <input type="text" name="school_name" id="school_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" value="{{ $lists->school_name }}" placeholder="Type product name" required="">
                    </div>     
                </div>
            </div>
            <div class="mt-4">
                <label for="points" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Points</label>
                <input type="number" name="points" id="points" value="{{ $lists->points }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type product name" required="">
            </div>
            <div class="my-4">
                <span class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">School Logo</span>
                <div class="flex justify-center items-center w-full">
                    <label for="school_logo" class="flex flex-col justify-center items-center w-full h-64 bg-gray-50 rounded-lg border-2 border-gray-300 border-dashed cursor-pointer dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                        ...
                        <input name="school_logo" id="school_logo" type="file" class="flex justify-center">
                    </label>
                </div>
            </div>
            ...
            <div class="relative w-full h-auto max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-6 text-center">
                        ...
                        <form method="POST" action="{{ url('delete_school', ['id' => $lists->id]) }}">
                            @csrf
                            @method('DELETE')
